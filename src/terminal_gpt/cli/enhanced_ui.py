...
class EnhancedUI:
    ...
    def print_warning(
        self, message: str, details: Optional[str] = None, persistent: bool = False
    ):
    ...
    def print_welcome(self):
        ...
        # Print subtitle
        subtitle = Text("AI-Powered Terminal Assistant", style="jengo.subtitle")
    ...
    def thinking_indicator(
        self, mode: str = "thinking", duration: Optional[float] = None
    ):
        """Create a thinking indicator context manager."""
        return ThinkingIndicator(self, mode, duration)

    def print_plugin_output(
        self, plugin_name: str, output_type: str, data: Any, title: Optional[str] = None
    ):
    ...
    def _print_plugin_text(self, data: str, title: str, color: str):
        ...
        panel = Panel(data, title=f"📄 {title}", border_style=color, padding=(1, 2))
        self.console.print(panel)
    ...
    def validate_terminal_size(self) -> Dict[str, Any]:
        ...
        if width < 80:
            issues.append(
                f"Terminal width ({width}) is below recommended 80 characters"
            )
            recommendations.append(
                "Consider increasing terminal width for better experience"
            )

        if height < 24:
            issues.append(f"Terminal height ({height}) is below recommended 24 lines")
            recommendations.append(
                "Consider increasing terminal height for better experience"
            )
        ...
        if not self.console.color_system:
            ...
            recommendations.append(
                "Enable color support in your terminal for better experience"
            )
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb):
        ...
        if elapsed > 5:  # Only show if it took significant time
            self.ui.print_status(
                StatusLevel.INFO, f"Completed in {elapsed:.2f}s", persistent=True
            )

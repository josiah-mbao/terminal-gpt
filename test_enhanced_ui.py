...
async def test_enhanced_ui():
    ...
    enhanced_ui.print_status(
        StatusLevel.SUCCESS, "API connection established", "All systems operational"
    )
    ...
    enhanced_ui.print_status(
        StatusLevel.ERROR, "Plugin execution failed", "File not found: /path/to/file"
    )

    enhanced_ui.print_status(
        StatusLevel.INFO, "System ready", "Ready to accept commands"
    )
    ...
    enhanced_ui.print_plugin_output(
        "file_system", "tree", tree_data, "Project Structure"
    )
    ...
    enhanced_ui.print_command_result(
        "sessions", True, "Retrieved 3 active sessions", 0.15
    )
    enhanced_ui.print_command_result(
        "stats", False, "Failed to retrieve statistics", 0.05
    )

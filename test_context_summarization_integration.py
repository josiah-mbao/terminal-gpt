    ...
    try:
        ...
        for i in range(25):
            messages.append(
                Message(
                    role="user",
                    content=f"User message {i+1}: This is a test message about coding and debugging.",
                )
            )
            messages.append(
                Message(
                    role="assistant",
                    content=f"Assistant response {i+1}: I understand your question about coding.",
                )
            )
        ...
        messages.append(
            Message(
                role="tool",
                content="Tool result: File operation completed successfully",
                name="read_file",
            )
        )
        ...
        if should_summarize:
            # Test summarization
            summary, recent_messages = await summarizer.summarize_conversation(
                conversation
            )
            ...
            print(
                f"Summary message content preview: {summary_message.content[:100]}..."
            )
            ...
            print(
                f"Summarized conversation has {len(summarized_conversation.messages)} messages"
            )
